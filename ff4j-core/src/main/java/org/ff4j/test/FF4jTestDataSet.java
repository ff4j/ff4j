...
import org.ff4j.feature.strategy.expression.ExpressionToggleStrategy;
...
public interface FF4jTestDataSet {
    ...
    default Map < String, FF4jUser> expectedUsers() {
        ...
        users.put(UJohn, new FF4jUser(UJohn).fisrtName("John").lastName("Connor")
                            .description("sample description if OK")
                            .addPermission(FF4jPermission.ADMIN_FEATURES)
                            .addRole("ADMINISTRATOR"));
        users.put(USarah, new FF4jUser(USarah).fisrtName("Sarah").lastName("Connor")
                .description("sample description if OK")
    ...
    default Map < String, FF4jRole> expectedRoles() {
        ...
        roles.put("ADMINISTRATOR", new FF4jRole("ADMINISTRATOR").grant(
        ...
        roles.put("SUPERUSER", new FF4jRole("SUPERUSER").grant(
                        FF4jPermission.TOGGLE_FEATURES, FF4jPermission.VIEW_FEATURES,
                        FF4jPermission.VIEW_PROPERTIES,  FF4jPermission.VIEW_AUDITTRAIL, 
                        FF4jPermission.VIEW_FEATUREUSAGE));
        roles.put("USER", new FF4jRole("USER").grant(
    ...
    default Map < String, Feature> expectedFeatures() {
        ...
        Feature f2 = new Feature(F2)
            ...
            .addCustomProperty(new PropertyInt("ppint", 12))
            .addCustomProperty(new PropertyDouble("ppdouble", 12.5))
            .addCustomProperty(new PropertyBoolean("ppboolean", true))
            .addCustomProperty(new PropertyString("ppstring", "hello"))
            .addCustomProperty(new PropertyListInt("ppstring", 12,13,14))
            .addCustomProperty(new PropertyLogLevel("myLogLevel",LogLevel.DEBUG))
            .addCustomProperty(new PropertyInt("digitValue", 1).addFixedValues(0,1,2,3))
            .addCustomProperty(new PropertyString("regionIdentifier", "NA").addFixedValues("NA", "APAC", "EMEA"));
        Map < FF4jPermission, FF4jGrantees > permissions = f2.getAccessControlList().getPermissions();
        permissions.put(FF4jPermission.FEATURE_TOGGLE, new FF4jGrantees(Util.setOf("pierre"), new HashSet<>()));
        permissions.put(FF4jPermission.FEATURE_VIEW, new FF4jGrantees(new HashSet<>(), Util.setOf("EVERYONE")));
        results.put(F2, f2);
